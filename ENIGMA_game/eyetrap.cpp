...
//デバイスの取得
	...
	//頂点バッファの生成
	pDevice->CreateVertexBuffer(sizeof(VERTEX_2D) * 4, D3DUSAGE_WRITEONLY, FVF_VERTEX_2D, D3DPOOL_MANAGED, &g_pVtxBuffEyeEyeTrap, NULL);

	VERTEX_2D* pVtx;//頂点情報のポインタ

	//頂点バッファをロックし、頂点情報へのポインタを取得
	g_pVtxBuffEyeEyeTrap->Lock(0, 0, (void**)&pVtx, 0);
	g_EyeTrap.pos = D3DXVECTOR3(SCREEN_WIDE / 4.0f, SCREEN_HEIGHT / 3.0f, 0.0f);
	pVtx[0].pos = D3DXVECTOR3(g_EyeTrap.pos.x - SCREEN_WIDE / 4.0f, g_EyeTrap.pos.y - SCREEN_HEIGHT / 3.0f, 0.0f);
	pVtx[1].pos = D3DXVECTOR3(g_EyeTrap.pos.x + SCREEN_WIDE / 4.0f, g_EyeTrap.pos.y - SCREEN_HEIGHT / 3.0f, 0.0f);
	pVtx[2].pos = D3DXVECTOR3(g_EyeTrap.pos.x - SCREEN_WIDE / 4.0f, g_EyeTrap.pos.y + SCREEN_HEIGHT / 3.0f, 0.0f);
	pVtx[3].pos = D3DXVECTOR3(g_EyeTrap.pos.x + SCREEN_WIDE / 4.0f, g_EyeTrap.pos.y + SCREEN_HEIGHT / 3.0f, 0.0f);
	//rhwの設定
	pVtx[0].rhw = 1.0f;
	pVtx[1].rhw = 1.0f;
	pVtx[2].rhw = 1.0f;
